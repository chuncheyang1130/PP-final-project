...
class ProjectController(app_manager.RyuApp):
    ...
    def install_paths(self, src, first_port, dst, last_port, ip_src, ip_dst):
        ...
        out = subprocess.check_output(['./executables/bellmanford_serial', f'./topo_txts/topo_{NODE_NUM}.txt', str(src), str(dst)]).decode('utf-8').strip().split('\n')
        print(out)
        print('shortest path by bellmanford_SERIAL: ', out[4])
        print('cost:', out[5].split(':')[1].split()[0])
        time = out[2].split('\t\t')[1].split('[')[1].split(']')[0]
        print(time, 'ms')
        
        out2 = subprocess.check_output(['./executables/bellmanford_thread', f'./topo_txts/topo_{NODE_NUM}.txt', str(src), str(dst)]).decode('utf-8').strip().split('\n')
        print('shortest path by bellmanford_THREAD: ', out2[4])
        print(out2)
        print('cost:', out2[5].split(':')[1].split()[0])
        time_thread = out2[2].split('\t\t')[1].split('[')[1].split(']')[0]
        print(time_thread, 'ms')
        
        paths = []
        tmp_list = []
        path_list = out2[4].split('->')
        for node in path_list:
            node = node.strip()
            node = int(node) 
            tmp_list.append(node)
        paths.append(tmp_list)
        with open(f'./time_data/bellman_time_{NODE_NUM}.txt', 'a') as file:
            file.write(f'{time} {time_thread}\n')
        print(f'Execution time wrote into "./time_data/bellman_time_{NODE_NUM}.txt"!')
